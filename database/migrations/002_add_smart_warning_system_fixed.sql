-- Smart Professional Warning System Migration - Fixed Version
...
-- Professional Smart Functions for Intelligence Engine

-- Calculate Phase Impact Score (for connected intelligence)
...
-- Predict Project Completion (advanced timeline forecasting)
...
-- Generate Budget Warning (professional financial intelligence)
...
-- Trigger for Automatic Budget Warning Generation
...
BEGIN
    ...
    FROM generate_budget_warning(NEW.project_id)
    LIMIT 1;
...
COMMENT ON TABLE warning_analytics IS 'Stores intelligent warning data with ML-like confidence scoring and risk assessment';
COMMENT ON TABLE phase_dependencies IS 'Manages phase relationships for connected intelligence and cascade analysis';
COMMENT ON TABLE resource_predictions IS 'Predictive resource management with burnout risk and efficiency forecasting';
COMMENT ON TABLE project_timeline_forecasts IS 'Advanced timeline predictions using multiple forecasting models';
COMMENT ON TABLE critical_path_analysis IS 'Professional critical path analysis with optimization recommendations';
COMMENT ON TABLE smart_notification_rules IS 'Intelligent notification system with escalation and pattern recognition';

COMMENT ON FUNCTION calculate_phase_impact_score(INTEGER) IS 'Calculates professional impact score for connected phase intelligence';
COMMENT ON FUNCTION predict_project_completion(INTEGER, VARCHAR) IS 'AI-powered project completion prediction with multiple forecast types';
COMMENT ON FUNCTION generate_budget_warning(INTEGER) IS 'Professional budget analysis with overrun prediction and confidence scoring';

-- Success message
SELECT 'Smart Professional Warning System installed successfully! 🧠✨' as status;