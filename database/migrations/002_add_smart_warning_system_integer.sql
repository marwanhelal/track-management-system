-- Smart Professional Warning System Migration - INTEGER Version
-- Compatible with INTEGER-based schema matching schema.sql
...
-- Professional Smart Functions for Intelligence Engine (INTEGER compatible)

-- Calculate Phase Impact Score (INTEGER compatible)
...
-- Predict Project Completion (INTEGER compatible with work_logs)
CREATE OR REPLACE FUNCTION predict_project_completion(project_id INTEGER, forecast_type VARCHAR(20) DEFAULT 'realistic')
RETURNS DATE AS $$
DECLARE
...
-- Generate Budget Warning (INTEGER compatible with work_logs)
CREATE OR REPLACE FUNCTION generate_budget_warning(project_id INTEGER)
...
BEGIN
    ...
    GROUP BY p.id, p.name, p.predicted_hours, p.actual_hours, p.status, p.created_at, p.created_by;
...
-- Trigger for Automatic Budget Warning Generation (INTEGER Compatible)
...
DECLARE
    ...
    project_int INTEGER;
BEGIN
    -- Get the project INTEGER from the phase
    SELECT pp.project_id INTO project_int
    FROM project_phases pp
    WHERE pp.id = NEW.phase_id;

    -- Check if this project needs a budget warning
    SELECT * INTO warning_result
    FROM generate_budget_warning(project_int)
    LIMIT 1;
...
COMMENT ON TABLE warning_analytics IS 'Stores intelligent warning data with ML-like confidence scoring and risk assessment (INTEGER support)';
COMMENT ON TABLE phase_dependencies IS 'Manages phase relationships for connected intelligence and cascade analysis (INTEGER support)';
COMMENT ON TABLE resource_predictions IS 'Predictive resource management with burnout risk and efficiency forecasting (INTEGER support)';
COMMENT ON TABLE project_timeline_forecasts IS 'Advanced timeline predictions using multiple forecasting models (INTEGER support)';
COMMENT ON TABLE critical_path_analysis IS 'Professional critical path analysis with optimization recommendations (INTEGER support)';
COMMENT ON TABLE smart_notification_rules IS 'Intelligent notification system with escalation and pattern recognition';

COMMENT ON FUNCTION calculate_phase_impact_score(INTEGER) IS 'Calculates professional impact score for connected phase intelligence (INTEGER phases)';
COMMENT ON FUNCTION predict_project_completion(INTEGER, VARCHAR) IS 'AI-powered project completion prediction with INTEGER support';
COMMENT ON FUNCTION generate_budget_warning(INTEGER) IS 'Professional budget analysis with INTEGER support for all entities';

-- Success message
SELECT 'Smart Professional Warning System (INTEGER Version) installed successfully! 🧠✨📊🔢👥' as status;